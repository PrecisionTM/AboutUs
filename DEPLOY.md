...
2. **Import into Vercel**
   ...
   - Select the shared repo
   - **Framework Preset**: Other
   - **Root Directory**: `vercel-about` ← ⚠️ THIS IS REQUIRED
...
- Set Root Directory to blank — it must be set to `vercel-about`
...
- [x] `--section-gap: clamp(2rem, 4vw, 3.5rem)` (content-driven, tightened from default)
- [x] All image paths are relative (`images/...`) — no absolute `/images/...` paths
- [x] Image assets: `images/carlie-cannon-rius.jpg` ✅ real photo confirmed (637,786 bytes)
...
- [x] CTA URLs match conventions.md (`/get-started/`)
- [x] All `position: absolute` layers inside parents with explicit non-viewport height
- [x] `object-fit: cover` applied to CEO photo
- [x] No `position: fixed` elements
- [x] All image assets present and non-zero bytes
